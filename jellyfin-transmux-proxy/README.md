...
A high-performance, specialized edge proxy for Jellyfin that replaces the heavy "FFmpeg-to-disk" pipeline with a fast, in-memory transmuxing engine. ⚡️
...
The **Jellyfin Transmux Proxy** optimizes this by intercepting these requests and handling the heavy lifting natively.
...
- **Concurrency**: Built on the **Tokio** async runtime, capable of handling hundreds of concurrent connections with minimal footprint.
- **Transmuxing**: Handles the transition from various containers to HLS-compliant fMP4 natively.
- **Transcoding**: Supported for **Audio** (Video transcoding is not implemented).
...
This project is in active development. It currently excels at **transmuxing** and **audio transcoding** (to aac only).
